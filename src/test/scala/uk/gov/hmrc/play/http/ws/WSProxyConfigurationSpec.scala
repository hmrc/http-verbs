...
class WSProxyConfigurationSpec extends WordSpecLike with Matchers with BeforeAndAfter {
  ...
  "If the proxyRequiredForThisEnvironment flag is not present, the WSProxyConfiguration apply method" should {
    ...
    "fail if no proxy is defined" in new FakeApplication() {
      a [ProxyConfigurationException] should be thrownBy WSProxyConfiguration("Dev.httpProxy")
    }

    "return the proxy configuration if the proxy is defined" in new FakeApplication(additionalConfiguration = proxyConfigWithFlagSetTo(None)) {
  ...
  "If the proxyRequiredForThisEnvironment flag is set to true, the WSProxyConfiguration apply method" should {
    ...
    "fail if no proxy is defined" in new FakeApplication(additionalConfiguration = proxyFlagConfiguredTo(value = true)) {
      a [ProxyConfigurationException] should be thrownBy WSProxyConfiguration("Dev.httpProxy")
    }

    "return the proxy configuration if the proxy is defined" in new FakeApplication(additionalConfiguration = proxyConfigWithFlagSetTo(Some(true))) {
      WSProxyConfiguration("Dev.httpProxy") shouldBe Some(proxy)
    }
  }

  "If the proxyRequiredForThisEnvironment flag is set to false, the WSProxyConfiguration apply method" should {
    "return None if no proxy is defined" in new FakeApplication(additionalConfiguration = proxyFlagConfiguredTo(value = false)) {
      WSProxyConfiguration("Dev.httpProxy") shouldBe None
    }

    "return None if the proxy is defined" in new FakeApplication(additionalConfiguration = proxyConfigWithFlagSetTo(Some(false))) {
      WSProxyConfiguration("Dev.httpProxy") shouldBe None
    }
  }
}
